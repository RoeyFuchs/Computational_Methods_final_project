...
scaler = StandardScaler() # normlize the data
scaler.fit(data_x)
...
for i in range(100):
    ...
    y_hat_super_result.append((np.sum((y_hat_super == test_data_y)) / len(test_data_y)) * 100)

print("svm: ", np.average(y_hat_svm_result))
print("lr: ", np.average(y_hat_lr_result))
print("ann: ", np.average(y_hat_ann_result))
print("super: ", np.average(y_hat_super_result))
